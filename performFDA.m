...
if option.doCrossValidation
    % search for the best value for lambda, the roughness penalty
    logLambda   = -5:1:5;
    gcvSave = zeros(length(logLambda),size(xNorm,3));
    dfSave  = gcvSave;
    for i = 1:length(logLambda)
        xFdPari = fdPar(basis,setup.Fd.penaltyOrder,10^logLambda(i));
        [~,dfi,gcvi] = smooth_basis(timespan,xNorm,xFdPari);
        gcvSave(i,:) = sum(gcvi,2);
        dfSave(i,:)  = dfi;
    end
    %  plot the results
    clf;
    ...
    ylabel('\fontsize{13} log_{10}(GCV(\lambda))');
    pause;
end


% ************************************************************************
%   Find minimum GCV value of number of bases
% ************************************************************************

if option.doCrossValidation2
    % search for the best value for lambda, the roughness penalty
    nBases = 50:50:800;
    gcvSave = zeros(length(nBases),size(xNorm,3));
    dfSave  = gcvSave;
    sseSave = gcvSave;
    for i = 1:length(nBases)
        xbasis = create_bspline_basis([tStart,tEnd],nBases(i),setup.Fd.basisOrder);
        xFdPari = fdPar(xbasis,setup.Fd.penaltyOrder,setup.Fd.lambda);
        [~,dfi,gcvi,~,ssErri] = smooth_basis(timespan,xNorm,xFdPari);
        gcvSave(i,:) = sum(gcvi,2);
        dfSave(i,:)  = dfi;
        sseSave(i,:) = sqrt(sum(ssErri,2)/(nJumps*(maxLength-dfi)));
    end
    %  plot the results
    clf;
    hold on;
    yyaxis left;
    plot(nBases,log10(gcvSave),'k-o');
    ylabel('\fontsize{13} log_{10}(GCV(n))');
    yyaxis right;
    plot(nBases,log10(dfSave),'r-o');
    plot(nBases,log10(nBases),'m:');
    ylabel('\fontsize{13} log_{10}(dF(n))');
    xlabel('\fontsize{13} n');
    hold off;
    pause;
end
...
% ************************************************************************
%   Find minimum performance variance for number of bases
% ************************************************************************

if option.doCrossValidation3
    % search for the best number of bases
    nBases = 50:50:800;
    rmsei = zeros(length(nBases),1);
    
    for i = 1:length(nBases)
        xbasis = create_bspline_basis( ...
            [tStart,tEnd],nBases(i),setup.Fd.basisOrder);
        xFdPari = fdPar(xbasis,setup.Fd.penaltyOrder,setup.Fd.lambda);
        [xFd,~,~,~,~] = smooth_basis(timespan,xNorm,xFdPari);      
        xPts = eval_fd(timespan,xFd);
        rmsei(i) = perf_rmse(xPts,xNorm,tFreq,1);
    end
    %  plot the results
    clf;
    plot(nBases,log10(rmsei),'k-o');
